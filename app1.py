...
def main():
    st.set_page_config(page_title="Probability of death on Titanic")
    overview = st.container()
    left, right = st.columns(2)
    prediction = st.container()
    st.image("https://miro.medium.com/max/1200/1*fBkTkunRJ88FdEXEcGU_fg.jpeg")
    with overview:
        st.title("Probability of death on Titanic")
    with left:
